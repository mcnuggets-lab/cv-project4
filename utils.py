...
def resize_images_bilinear(X, height_factor=1, width_factor=1, target_height=None, target_width=None,
                           data_format='default'):
    """
    Resizes the images contained in a 4D tensor of shape by a factor of (height_factor, width_factor)
    :param X: Input layer
    :param height_factor: positive integer
    :param width_factor: positive integer
    :param target_height: resize original height by height_factor if target_height is not given
    :param target_width: resize original weight by height_factor if target_weight is not given
    :param data_format: default / channels_first / channels_last
    :return: output layer
    """
    if data_format == 'default':
        data_format = K.image_data_format()
    if data_format == 'channels_first':  # [batch, channels, height, width]
        ...
        return X
    elif data_format == 'channels_last':  # [batch, height, width, channels]
        original_shape = K.int_shape(X)
        if target_height and target_width:
            new_shape = tf.constant(np.array((target_height, target_width)).astype('int32'))
        else:
